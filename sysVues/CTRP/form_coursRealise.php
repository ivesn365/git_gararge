...
if(isset($_GET['id'])){
    ...
    <div class="container mt-5">
        <div class="card shadow-sm border-0">
            <div class="card-body">
                <h4 class="card-title text-center mb-4">Course réalisée</h4>
                <div id="loader"></div>
                <form id="form-course">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            <label for="destination" class="form-label">Nombre des courses réalisées</label>
                            <input type="number" class="form-control" value="<?=$course->getCourseRealise()?>"  id="coursesRealiser" name="coursesRealiser" required>
                        </div>
                        <div class="col-md-6">
                            <label for="destination" class="form-label">Kilomètre parcouru</label>
                            <input type="number" class="form-control" value="<?=$course->getIndexKm()?>"  id="indexKm" name="indexKm" required>
                        </div>
                        <div class="col-md-6">
                            <label for="destination" class="form-label">Carburant consommé</label>
                            <input type="number" class="form-control" value="<?=$course->getCarburant()?>"  id="carburant" name="carburant" required>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#form-course').on('submit', function(e) {
                ...
                const coursesRealiser = $('#coursesRealiser').val();
                const courses = $('#courses').val();
                const indexKm = $('#indexKm').val();
                const carburant = $('#carburant').val();
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        btn_send_course_prevision_confirmer: true,
                        vehicule: vehicule,
                        chauffeur: chauffeur,
                        date: date,
                        ...
                        courses:courses,
                        id:<?=$id?>,
                        coursesRealiser:coursesRealiser,
                        indexKm:indexKm,
                        carburant:carburant
...
<?php   }
    ...
    header("Location:page-previsions");
    ?>
